    ...
    public static class ConsultClient
    {
        ...
        public static async Task<ResponseService<bool>> UnRegisterService(string id_service = null)
        {
            try
            {
                ...
                foreach (string addressConsul in listAddressConsul)
                {
                    ...
                    await client.Agent.ServiceDeregister("BasicIC_Email-localhost:44333");
                }
        ...
        public static async Task<ResponseService<string>> GetInfoServiceConsult(string nameService)
        {
            try
            {
                ...
                foreach (var serviceValue in services.Response.Select(q => q.Value))
                {
                    // get status service
                    var healthService = services_health.Response.FirstOrDefault(item => item.Value.ServiceName == serviceValue.Service);
                    ...
                    // get url service
                    if (serviceValue.Service.Equals(nameService) && statusService.Equals(Constants.SERVICE_CONSULT_GOOD_HEALTH))
                    {
                        string baseUrl = $"{serviceValue.Address}:{serviceValue.Port}";
        ...
        public static async Task<ResponseService<bool>> SendTopic(TopicParam param)
        {
            try
            {
                ...
                if (stateSource == Constants.STATE_SOURCE_PRODUCTION) return new ResponseService<bool>().BadRequest("This is state production!");

                ProducerWrapper<Object> _producer = new ProducerWrapper<Object>();
                await _producer.CreateMess(param.topic, param);
        ...
        /// <summary>
        /// unsubribe list topic
        /// </summary>
        /// <param name="param"></param>
        /// <returns></returns>
        public static ResponseService<bool> UnSubscribeTopic(TopicSubscribe param)
        {
            try
            {
                _logger.LogInfo(CommonFunc.GetMethodName(new System.Diagnostics.StackTrace()));
                ...
                var cw = new ConsumerWrapper();
                var topicsCurrent = cw.GetCurrentConsumer().Subscription;
                var topicNew = topicsCurrent.Except(param.list_topic);
                cw.UpdateConsumerTopic(topicNew);
        ...
        ///// <param name="param"></param>
        ///// <returns></returns>
        public static ResponseService<bool> SubscribeTopic(TopicSubscribe param)
        {
            try
            {
                _logger.LogInfo(CommonFunc.GetMethodName(new System.Diagnostics.StackTrace()));
                ...
                var cw = new ConsumerWrapper();
                var topicsCurrent = cw.GetCurrentConsumer().Subscription;
                topicsCurrent.AddRange(param.list_topic);
                cw.UpdateConsumerTopic(topicsCurrent);
